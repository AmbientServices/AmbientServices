...
//                await cache.Store(keyName6, this, TimeSpan.FromSeconds(50));
        ...
        public async Task CacheRefresh()
        {
            ...
            using (new ScopedLocalServiceOverride<IAmbientSettingsSet>(localSettingsSet))
            {
                ...
                await cache.Store<TestCache>("CacheRefresh1", this, TimeSpan.FromSeconds(1));

                AmbientClock.SkipAhead(TimeSpan.FromMilliseconds(1100));

                ret = await cache.Retrieve<TestCache>("CacheRefresh1", null);
                Assert.IsNull(ret);
                await cache.Store<TestCache>("CacheRefresh1", this, TimeSpan.FromMinutes(10));
                ret = await cache.Retrieve<TestCache>("CacheRefresh1", null);
                Assert.AreEqual(this, ret);
                await Eject(cache, 1);

                await cache.Store<TestCache>("CacheRefresh2", this);
                ret = await cache.Retrieve<TestCache>("CacheRefresh2", null);
                Assert.AreEqual(this, ret);
                await cache.Store<TestCache>("CacheRefresh3", this);
                ret = await cache.Retrieve<TestCache>("CacheRefresh3", null);
                Assert.AreEqual(this, ret);
                await cache.Remove<TestCache>("CacheRefresh3");
                ret = await cache.Retrieve<TestCache>("CacheRefresh3", null);
                Assert.IsNull(ret);
