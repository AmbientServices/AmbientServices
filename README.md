...
### Disposable Entries
The local cache is the only cache in the library that may hold disposable entries (`IDisposable` or `IAsyncDisposable`), because its entries never leave the process: exactly one instance exists, it is the instance handed back, and so an owner can always be named.  The caller picks the owner at store time with `disposeWhenDiscarding`:

* `true` — the cache owns the entry and disposes it whenever it discards it (replaced by a later store under the same key, expired, evicted, or cleared).  To avoid disposing something still in use, retrieval is a hand-off: the entry is removed from the cache and *not* disposed, so exactly one client holds it at a time.  `Remove` hands it back the same way.
* `false` — the caller owns the entry for its whole life.  Retrieval is non-destructive and may hand the same instance to any number of callers, and the cache never disposes it, so an entry the cache silently discards leaks unless the caller still holds a reference.  Treat such entries as shared mutable state.

Either way, if no cache service is in effect the store is simply discarded and the item is left undisposed for the caller to deal with, and an item the caching arguments rule out entirely is disposed only when the cache was given ownership of it.  An entry that implements both disposal interfaces may be disposed through either one, so it must be safe to dispose either way; the default implementations prefer `IAsyncDisposable` and invoke exactly one of the two, on every target framework.

**Disposable entries must never be stored in `IAmbientSharedCache`, `AmbientTwoStageCache`, or an `IAmbientAtomicCache` whose `IsShared` is true.**  Those entries may leave the process: storing serializes the item, so a later retrieval may hand back a fresh copy — here or on another machine — and neither side can name an owner for those copies.  The store cannot know what a handle inside a blob refers to, and the caller never learns how many copies exist or when the last reader finished with one.  These interfaces therefore promise nothing in either direction: a caller may neither rely on the cache disposing an entry nor assume it will leave one alone, since an in-process realization may well dispose what it evicts.  The signatures reflect that — no `disposeWhenDiscarding` option, and `Remove` hands nothing back — so ownership never moves.  Nothing enforces the rule (storing a disposable item raises no error); it fails later, as a leaked handle, a copy that deserialized into a useless one, or an `ObjectDisposedException` from a realization that disposed what it evicted.

The one exception is a type that implements a disposal interface without owning anything whose release matters — nothing unmanaged, pooled, or external — and that survives a serialization round trip intact, such as a `MemoryStream`-backed payload.  Leaving such an entry undisposed costs nothing, so caching it is safe; the rule is about entries whose disposal is required for correctness.
...
Disposable values may be cached only when `IsShared` is false; a shared realization serializes entries out of the process and so falls under the prohibition described above.  A local realization takes dispose responsibility automatically — there is no opt-in flag, because discarding values is inherent to add-or-update under contention — and disposes a factory result that lost the race to install, an update that lost its compare-and-swap, a value the caching arguments ruled out, an entry displaced by a later write, and every entry it removes, ejects, or clears.  That is what keeps a factory allocating a disposable from leaking one on every lost race.  Retrieval, though, is never a hand-off: `GetOrAdd` and `VersionedGet` return the live instance and leave the entry installed, so the cache can dispose an instance a caller is still holding and that caller then sees an `ObjectDisposedException`.  Cache disposable values here only when your use of them finishes inside the operation that fetched them, or when disposal mid-use is harmless; when an entry must survive in one owner's hands, use `IAmbientLocalCache` with `disposeWhenDiscarding: true`, which gives up the entry on the first hit for exactly that reason.
...
```
### Default Implementation
The default implementation logs messages to the `System.Diagnostics.Trace` system,
asynchronously buffers the log messages and flushes them in batches out to hourly rotating log files located in the system temp folder in files prefixed with "BasicAmbientLogger" and suffixed with the hour and a ".log" extension.
...
That notification can come in several forms.  First, the `DisposeResponsibility.ResponsibilityNotDisposed` event can be subscribed to for immediate handling (for example `Debugger.Break()` during interactive debugging).
...
Care should be taken in the event handler for the `DisposeResponsibility.ResponsibilityNotDisposed` event to avoid throwing exceptions, as this could cause the application to crash because it is called by the garbage collector during finalization.
...
## Language and Tools
AmbientServices is written in C#, using .NET Standard 2.0, .NET 8.0-10.0.  Unit tests are written in .NET 10.0.
The code can be built using either Microsoft Visual Studio 2026+, Microsoft Visual Studio Code, or .NET Core command-line utilities.
