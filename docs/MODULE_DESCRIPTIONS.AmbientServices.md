...
- **A Pin and its linkers.** `StatusResults` carries the library's first `<pin>`: the cross-server interchange conventions of the status system — a leading slash on a target name marking a shared system that summarization re-roots, target names matching exactly across every source that reports one backend, and an underscore prefix declaring a property value sensitive. They are pinned by the Pin's own test, not by looking low-level: results are forwarded between servers and summarized by a build that need not match the one that produced them, so moving either rule stops already-deployed reporters from merging with current ones. `StatusChecker`, `StatusResultsBuilder`, and `StatusResultsOrganizer` must agree with it, and each carries a bare `<pin><see cref="StatusResults"/></pin>` — a link, never a restatement. Note that the Pin also records that the underscore convention is *unenforced*: it is a promise between whoever populates results and whoever renders them, which is precisely why it needs writing down.
