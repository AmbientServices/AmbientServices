...
/// </plan>
/// <pin>
/// The cross-server interchange conventions of the status system.  They are frozen because results are forwarded between servers and summarized somewhere other than where they were measured, by a build that need not be the same version as the one that produced them.
/// A leading slash on <see cref="TargetSystem"/> marks a shared system that summarization re-roots as top-level; its absence names a subsystem of the parent node.  Target names for one backend must match exactly across every source reporting it.  Move either rule and already-deployed reporters stop merging with current ones: one backend surfaces as several unrelated systems, farm-wide rollups split silently, and a condition that should have aggregated into a single failure is reported as a handful of separately-acceptable nodes.  <see cref="StatusResultsOrganizer"/> implements the re-rooting and must agree.
/// A property name beginning with an underscore declares its value sensitive and not publicly shareable (user or machine names, network addresses).  Nothing in this library enforces it — it is a promise between whoever populates results and whoever renders them — so changing the marker, or the meaning of the marker, silently publishes sensitive values through every renderer written to trust it.
/// </pin>
/// <priority>
