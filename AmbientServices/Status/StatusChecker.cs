    ...
    public abstract class StatusChecker : IDisposable
    {
        ...
        internal protected void SetLatestResults(StatusResults newResults)
        {
            if (newResults != null)
            {
                ...
                Status.Logger.Log($"{newResults.TargetSystemDisplayName}: {newResults.Report?.Alert}", "Results", level: (newResults.Report?.Alert?.Rating < StatusRating.Okay ? AmbientLogLevel.Warning : AmbientLogLevel.Verbose);
