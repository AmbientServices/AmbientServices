    ...
    public abstract class StatusChecker : IDisposable
    {
        ...
        internal protected void SetLatestResults(StatusResults newResults)
        {
            if (newResults != null)
            {
                ...
                Status.Logger.Filter("Results", newResults.Report?.Alert?.Rating < StatusRating.Okay ? AmbientLogLevel.Warning : AmbientLogLevel.Verbose)?.Log($"{newResults.TargetSystemDisplayName}: {newResults.Report?.Alert}");
