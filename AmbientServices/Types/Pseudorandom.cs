...
/// </summary>
/// <remarks>
/// <pitch>Reproducible pseudorandom data of many shapes (integers of every width, ranged values, skewed usually-small values, doubles, decimals, booleans, GUIDs, byte arrays) from an explicit, copyable seed — built for tests and simulations that need replayable randomness, with better small-divisor behavior than <see cref="Random"/>.  Never use it for cryptographic or security-sensitive purposes.</pitch>
/// <pledge>
/// Determinism is the contract: two instances with the same seed making the same sequence of calls produce identical values, a <see cref="Clone"/> continues identically to its source, and equality is seed-state equality — so a seed (or a cloned instance) fully captures a reproducible stream.
/// Every value produced advances the seed; ranged forms return values in [inclusive lower, exclusive upper) and throw when the bounds are inverted; the "usually small" forms cover the full stated range but concentrate results near zero.  Seeds from <see cref="Next"/> or seed-generating construction are distinct in practice even when obtained simultaneously.
/// Instances are mutable and not thread-safe; share streams across threads by giving each thread its own instance or seed.
/// </pledge>
/// <plan>
/// The generator multiplies the incrementing seed by a large prime and fully reverses the bit order (log-n swap cascade), so consecutive seeds produce well-scattered values without banding when taken modulo small constants; each width has its own prime/cascade.  Ranged and typed forms are thin arithmetic over those cores, and the usually-small forms randomize a divisor and down-shift to bias toward zero.
/// Seed generation for <see cref="Next"/> mixes <see cref="Environment.TickCount"/>, an <see cref="AmbientStopwatch"/>'s high-frequency ticks, and an interlocked global counter — collision-resistant across threads without locking.  All state is one 64-bit seed, making instances cheap to create, copy, and compare.
/// </plan>
/// <priority>
/// 1. Reproducibility over unpredictability: the entire state is one seed, a clone continues its source's stream exactly, and equality is seed equality — so a failing test or simulation can be replayed from a single number.  That is precisely what disqualifies it for anything security-sensitive, and why the Pitch says so outright rather than leaving it to be inferred. (public)
/// 2. Even distribution under small divisors over generator speed: values are produced by multiplying the seed by a large prime and fully reversing the bit order, because the cheaper generators band visibly when taken modulo small constants — which is exactly what test data does to them. (public)
/// 3. Cheap instances over thread safety: state is a single 64-bit seed with no synchronization, so instances are trivially created, copied, and compared, and sharing one across threads is the caller's error to avoid by giving each thread its own. (public)
/// </priority>
