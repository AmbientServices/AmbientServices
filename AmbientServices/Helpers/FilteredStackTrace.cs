...
/// <priority>
/// 1. A readable trace that leaks nothing over a complete one: framework frames, wrapper namespaces, library namespace prefixes, and build-machine source paths are all removed, which is deliberate destruction of real diagnostic information.  A trace nobody reads diagnoses nothing, and a trace pasted into a ticket should not disclose where the binary was built. (public)
/// 2. Keeping each transition into filtered code visible over maximum shortening: frames in a clustered namespace are dropped except the first of each consecutive run, so the trace still shows where control entered framework code rather than silently stitching unrelated frames together.  Full removal would be shorter and would make the result misleading. (public)
/// 3. Capture cost matching the base class over filtering eagerly: the filtered view is built lazily on first read, so a trace that is captured and never rendered costs exactly what <see cref="StackTrace"/> costs.  Traces are captured far more often than they are printed. (private)
/// </priority>
/// </remarks>
public class FilteredStackTrace : StackTrace
{
    /// <summary>
    /// Gets the current filtered stack trace as a string.
    /// Intended as a replacement for Environment.StackTrace.
    /// </summary>
    public static string Current
    {
        [MethodImpl(MethodImplOptions.NoInlining)] // Prevent inlining from affecting where the stacktrace starts
        get => new FilteredStackTrace(1, true).ToString();
    }
    ...
    public FilteredStackTrace(int skipFrames, bool needFileInfo)
        : base(skipFrames + 1, needFileInfo)
    {
        _filteredFrames = FilterFrames();
    }
    /// <summary>
    ...
    public FilteredStackTrace(Exception e, int skipFrames, bool needFileInfo)
        : base(e, skipFrames + 1, needFileInfo)
    {
        _filteredFrames = FilterFrames();
    }
    /// <summary>
