...
namespace AmbientServices
{
    ...
    public class AmbientCache<TOWNER>
    {
        ...
        public Task<T?> Retrieve<T>(bool localOnly, string itemKey, TimeSpan? refresh = null, CancellationToken cancel = default(CancellationToken)) where T : class
        {
            IAmbientCache? cache = _explicitCache ?? _Cache.Local;
            if (cache == null) return Task.FromResult<T?>(null);
            return cache.Retrieve<T>(localOnly, _cacheKeyPrefix + itemKey, refresh, cancel);
        }
        /// <summary>
